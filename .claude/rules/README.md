...
| 50 | `graphql.md` | `**/schema.graphqls`, `**/Resolver/**` | Domain-specific, narrow path scope — same tier as `api.md`/`database.md`. GraphQL resolvers are a separate API surface from REST/webapi.xml, so overlap with `api.md` is unlikely in practice. |
...
| 40 | `markdown.md` | `**/*.md` | Format-specific, applies repo-wide — same tier as `magento-xml.md`. No glob overlap (`.md` vs `.xml`/`.xsd`), so no conflict between them. |
