# XAMPP / Magento 2 install — native Windows, no Docker required.
#
# Run in PowerShell from anywhere (it locates the project root itself):
#   .\scripts\install-magento-xampp.ps1
# or via: make magento-install
#
# Prerequisites:
#   - XAMPP: Apache + MySQL running
#   - PHP 8.2+ extensions enabled (see script output)
#   - Composer installed
#   - Magento Marketplace keys in auth.json (or docker/.env)
#   - OpenSearch/Elasticsearch on localhost:9200 (see notes below)
#
# Idempotent: safe to re-run. If bin/magento already exists it runs
# setup:upgrade instead of setup:install.

$ErrorActionPreference = 'Stop'

# This script lives in <project root>/scripts/ — resolve and cd to the
# project root so every relative path below (composer.json, app/code, etc.)
# works regardless of where the script was invoked from.
...
# this assumes the project lives directly under XAMPP's htdocs (or an Alias
# with the same name points at this folder's pub/). Rename the folder or add
# a matching Apache Alias if that's not the case.
...
<IfModule mod_rewrite.c>
    ...
    RewriteRule ^`$ pub/ [L]
    RewriteCond %{REQUEST_URI} !^/$ProjectName/pub/
    RewriteRule ^(.*)`$ pub/`$1 [L]
</IfModule>
DirectoryIndex index.php
"@ | Set-Content (Join-Path $ProjectRoot '.htaccess') -Encoding ASCII
}

# Detect the module under app/code (skip the placeholder Vendor/ModuleName),
# mirroring scripts/install-magento.sh's Docker-path detection logic.
function Get-ModuleId {
    if (-not (Test-Path 'app/code')) { return $null }
    foreach ($vendorDir in Get-ChildItem -Path 'app/code' -Directory) {
        foreach ($moduleDir in Get-ChildItem -Path $vendorDir.FullName -Directory) {
            if ("$($vendorDir.Name)/$($moduleDir.Name)" -eq 'Vendor/ModuleName') { continue }
            return "$($vendorDir.Name)_$($moduleDir.Name)"
        }
    }
    return $null
}
...
if (-not (Test-Path 'auth.json')) {
    Write-Host 'Create auth.json (copy auth.json.example) with your Magento Marketplace keys, or add keys to docker/.env' -ForegroundColor Red
    exit 1
}
...
Say 'Ensuring magento/product-community-edition is required'
if (-not (Select-String -Path 'composer.json' -Pattern 'magento/product-community-edition' -Quiet)) {
    # Pinned to match this repo's Docker path (scripts/install-magento.sh) and this
    # machine's PHP 8.2.12 — Magento 2.4.9 requires PHP 8.4+, so don't jump to latest
    # without upgrading PHP first (see docs/install-magento/versions.md).
    php -r '
        $f = "composer.json";
    ...
    '
    Write-Host 'Added magento/product-community-edition 2.4.7-p8 to composer.json' -ForegroundColor Green
}
...
$backupDir = Join-Path $ProjectRoot '_install_conflict_backup'
...
Magento 2.4 requires a search engine. Pick ONE option:
  ...
  B) Standalone OpenSearch for Windows, unzipped to C:\opensearch\opensearch-2.11.0
     (started automatically by scripts/start-magento.ps1).

  C) Install Elasticsearch 7/8 for Windows separately.

Then re-run: .\scripts\install-magento-xampp.ps1
...
if (-not $moduleId) {
    Write-Host 'No module found under app/code/ (only the Vendor/ModuleName placeholder). Run "make init" first if this is a new project.' -ForegroundColor Yellow
}
